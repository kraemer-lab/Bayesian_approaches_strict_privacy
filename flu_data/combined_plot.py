...
mvn_a = az.extract(idata_mvn)['alpha'].values.mean(axis=0)
mvn_b = az.extract(idata_mvn)['beta'].values.mean(axis=0)

meta_a = az.extract(idata_meta)['alpha'].values.mean(axis=0)
meta_b = az.extract(idata_meta)['beta'].values.mean(axis=0)

dir_a = az.extract(idata_dir)['alpha'].values.mean(axis=0)
dir_b = az.extract(idata_dir)['beta'].values.mean(axis=0)

mu_mvn = az.extract(idata_mvn)['mu'].values.mean(axis=0)
sig_mvn = az.extract(idata_mvn)['sigma'].values.mean(axis=0)

mu_mvn = az.extract(idata_mvn)['mu'].values.mean(axis=0)
sig_mvn = az.extract(idata_mvn)['sigma'].values.mean(axis=0)

mu_meta = az.extract(idata_meta)['mu'].values.mean(axis=0)
sig_meta = az.extract(idata_meta)['sigma'].values.mean(axis=0)
tau_meta = az.extract(idata_meta)['tau'].values.mean(axis=0)
the_meta = az.extract(idata_meta)['theta'].values.mean(axis=0)

mu_dir = az.extract(idata_dir)['mu'].values.mean(axis=0)
sig_dir = az.extract(idata_dir)['sigma'].values.mean(axis=0)

dir_a = mu_dir**2 / sig_dir**2
dir_b = mu_dir / sig_dir**2
...
ax1.text(mu_dir.mean() + 5, 0.2, r"$OVL_{MvN}$ = " + str(ovl_mvn_pdf), size=14)
ax1.text(mu_dir.mean() + 5, 0.15, r"$OVL_{Meta}$ = " + str(ovl_meta_pdf), size=14)
...
sns.kdeplot(mu_dir, c='steelblue', lw=4, ax=ax3, bw_adjust=1)
...
ax3.set_xticks(np.arange(2, 4 + 1, 1))
ax3.set_xlim(2,5)
...
ax4.set_xlim(0,12)
ax4.set_xticks(np.arange(0, 12, 5))
...
ax5.set_xlim(0,12)
ax5.set_xticks(np.arange(0, 12, 5))
...
ax6.set_xlim(0,12)
ax6.set_xticks(np.arange(0, 12, 5))

# Add a suptitle and save the figure
plt.suptitle("H7N9 Incubation Period Estimation (Comparison)", size=20, y=0.99)
plt.tight_layout()
...
names = [r"$\bf{A.}$ Direct Sampling Gamma Distribution (H7N9)",
         r"$\bf{B.}$ MvN-approx Gamma Distribution (H7N9)",
         r"$\bf{C.}$ Meta-analysis Gamma Distribution (H7N9)",] 
...
mvn_theta = mvn_theta.swapaxes(0,1)

meta_theta = az.extract(idata_meta)['theta'].values

dir_theta = az.extract(az.from_netcdf("./direct_analysis_idata_sites.nc"))['mu'].values

n_sites = len(mvn_theta[0])
...
names = [r"$\bf{A.}$ Direct Sampling (H7N9)",
         r"$\bf{B.}$ MvN-approx (H7N9)",
         r"$\bf{C.}$ Meta-analysis (H7N9)",] 
...
for i in range(len(ax)-1):
    ...
    tmean1 = theta.mean(axis=2)[0,:]
    tmean2 = theta.mean(axis=2)[1,:]
    thdi1 = az.hdi(theta[0,:].T, hdi_prob=0.9).T
    thdi2 = az.hdi(theta[1,:].T, hdi_prob=0.9).T
    
    ax[i].errorbar(tmean1, np.arange(n_sites), xerr=[tmean1 - thdi1[0], thdi1[1] - tmean1],
                 fmt='o', capsize=5, label='Site $μ_1$')
    ax[i].axvline(x=tmean1.mean(), color='r', linestyle='--', label='Overall $μ_1$')
    ax[i].errorbar(tmean2, np.arange(n_sites), xerr=[tmean2 - thdi2[0], thdi2[1] - tmean2],
                 fmt='o', capsize=5, label='Site $μ_2$')
    ax[i].axvline(x=tmean2.mean(), color='b', linestyle=':', label='Overall $μ_2$')
