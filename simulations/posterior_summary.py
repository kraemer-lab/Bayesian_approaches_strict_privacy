...
for i in range(len(dfs_sorted[1:])):
    # lamz_mu = summaries[i].loc['lamz']['mean']
    # lamz_sd =summaries[i].loc['lamz']['sd']
    # lam_mu = summaries[i].loc['lam']['mean']
    # lam_sd = summaries[i].loc['lam']['sd']
    # sig_mu = summaries[i].loc['sigma']['mean']
    # sig_sd = summaries[i].loc['sigma']['sd']
    alpha_mu = summaries[i].loc['alpha']['mean']
    alpha_sd = summaries[i].loc['alpha']['sd']
    mu_mu = summaries[i].loc['mu']['mean']
    mu_sd = summaries[i].loc['mu']['sd']
    if i+1 < 10:
        n = "0"+str(i+1)
    else:
        n = str(i+1)
    ...
    with pm.Model() as mod:
        # lamz = pm.Normal('lamz', mu=lamz_mu, sigma=lamz_sd)
        # lam = pm.Normal('lam', mu=lam_mu, sigma=lam_sd)
        # sigma = pm.TruncatedNormal('sigma', mu=sig_mu, sigma=sig_sd, lower=0)
        # mu = pm.Deterministic("mu", pm.math.exp(lam + lamz*sigma))
        mu = pm.TruncatedNormal('mu', mu=mu_mu, sigma=mu_sd, lower=1)
        alpha = pm.TruncatedNormal('alpha', mu=alpha_mu, sigma=alpha_sd, lower=0)
...
ax.set_title("TN Approximation Posterior CDF")
plt.tight_layout()
plt.savefig("posterior_summary_posterior.png", dpi=600)
plt.show()
plt.close()
...
# ax.set_ylabel("Probability")
# ax.set_xlabel("Incubation Days")
# ax.set_title("Summary Approach Posterior CDF")
# plt.tight_layout()
# plt.savefig("posterior_summary_posterior.png", dpi=600)
# plt.show()
# plt.close()
...
ax[1].plot(x2, cdf_mean, color='purple', linestyle="--", linewidth=3, label="μ mean TN Approx.")
ax[1].legend()
