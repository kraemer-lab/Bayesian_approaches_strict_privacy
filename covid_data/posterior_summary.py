...
for i in range(len(dfs_sorted[1:])):
    mu_m = summaries[i].loc['mu']['mean']
    mu_sd = summaries[i].loc['mu']['sd']
    sigma_m = summaries[i].loc['sigma']['mean']
    sigma_sd = summaries[i].loc['sigma']['sd']
    ...
    names.append(name)
    left_lower = dfs_sorted[1:][i].left_lower.values
    left_upper = dfs_sorted[1:][i].left_upper.values
    right_lower = dfs_sorted[1:][i].right_lower.values
    right_upper = dfs_sorted[1:][i].right_upper.values
    left_interval = np.array([[x,y] for x,y in zip(left_lower, right_lower) if x!=y])
    left_exact = np.array([[x] for x,y in zip(left_lower, right_lower) if x==y])
    right_interval = np.array([[x,y] for x,y in zip(left_upper, right_upper) if x!=y])
    right_exact = np.array([[x] for x,y in zip(left_upper, right_upper) if x==y])
    with pm.Model() as mod:
        mu = pm.TruncatedNormal('mu', mu_m, mu_sd, lower=1) 
        sigma = pm.TruncatedNormal('sigma', sigma_m, sigma_sd, lower=0) 
...
#ax[0,0].text(15, 0.1, "H² = "+str(H2_pdf))
